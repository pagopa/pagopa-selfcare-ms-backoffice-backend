...
public class RestExceptionsHandler {
    ...
    ResponseEntity<Problem> handleThrowable(Throwable e) {
        ...
        return ProblemMapper.toResponseEntity(new Problem(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage()));
    }
    ...
    ResponseEntity<Problem> handleHttpMediaTypeNotAcceptableException(HttpMediaTypeNotAcceptableException e) {
        log.warn(e.toString());
        return ProblemMapper.toResponseEntity(new Problem(HttpStatus.NOT_ACCEPTABLE, e.getMessage()));
    }
    ...
    ResponseEntity<Problem> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
        log.warn(e.toString());
        return ProblemMapper.toResponseEntity(new Problem(HttpStatus.METHOD_NOT_ALLOWED, e.getMessage()));
    }
    ...
    ResponseEntity<Problem> handleBadRequestException(Exception e) {
        log.warn(e.toString());
        return ProblemMapper.toResponseEntity(new Problem(HttpStatus.BAD_REQUEST, e.getMessage()));
    }


    @ExceptionHandler({MethodArgumentNotValidException.class})
    ResponseEntity<Problem> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        final Problem problem = new Problem(HttpStatus.BAD_REQUEST, "Validation failed");
